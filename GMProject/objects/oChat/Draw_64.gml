	...
	switch (name) {
		case "messages":
			...
			for (var offset = 0, i = 0; i < array_length(chat); i++) {
				...
				if (mouse_in_area_gui([_x, _y, _x + _w, _y + _h]) and mouse_in_area_gui([_x + 2, _y + over + offset, _x + 2 + string_width(msg.player), _y + over + offset + strh]) and device_mouse_check_button_pressed(0, mb_right)) {
